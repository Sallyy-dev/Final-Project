...
const orderService = require('../controllers/cancel.order.controller');
const createOrder = async (req, res) => {
  ...
  try {
    for (const item of items) {
      if (!item.categoryItem || !item.quantity || !item.price) {
        return res
          .status(400)
          .json({
            message: "Each item must have foodItem, quantity, and price",
          });
      }
      if (item.quantity <= 0) {
        return res
